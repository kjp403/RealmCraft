...
##   `unit`   float  cells per authored cell. Every distance and noise size here
##                  is authored against a 1x map, so a level built at the biome
##                  builder's 5x scale must pass its scale or the soot band ends
##                  up four cells wide on a map five times larger — invisible.
static func paint_slate(
	...
	var unit: float = maxf(float(cfg.get("unit", 1.0)), 1.0)
	var n: int = 0
	for cell: Vector2i in floor_mask.keys():
		if skip.has(cell):
			continue
		...
		var d: float = clampf(float(int(depth.get(cell, 4))) / (4.0 * unit), 0.0, 1.0)
		# A slow stain field on top, so the value drifts in blotches the size of
		# a chamber rather than flickering per tile.
		var stain: float = MapKit.fbm(float(cell.x), float(cell.y), 13.0 * unit, 3, seed_value)
...
static func pave(
	...
	var u: float = maxf(unit, 1.0)
	var margin: int = int(round(u))
	var done: Dictionary = {}
	for cell: Vector2i in region.keys():
		if not floor_mask.has(cell):
			continue
		if int(depth.get(cell, 1 << 20)) <= margin:
			continue
		if wear > 0.0 and MapKit.fbm(float(cell.x), float(cell.y), 4.0 * u, 2, seed_value) > 1.0 - wear:
			continue
		plate(ground, cell)
		done[cell] = true
	return done
