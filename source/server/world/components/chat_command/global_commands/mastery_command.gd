extends ChatCommand
## Set a weapon mastery level for testing. Omit level (or pass the cap) to max it.
...
func _init() -> void:
	...
	command_usage = "/mastery <self|@account|#id> <category|all> [level]"
...
func execute(args: PackedStringArray, peer_id: int, server_instance: ServerInstance) -> String:
	...
	if args.size() == 4:
		...
		if level < 1 or level > PlayerResource.MASTERY_LEVEL_CAP:
			return "Mastery level must be between 1 and %d." % PlayerResource.MASTERY_LEVEL_CAP
...
func _resolve_category(token: StringName) -> StringName:
	...
	for category: StringName in MasteryService.trees():
		...
		if String(tree.display_name).to_lower() == String(token):
			return category
		# Common shorthand: "magic" → wand, "archery" → bow, etc.
		if String(category) == String(token):
			return category
	# A few friendly aliases beyond display names.
	match String(token):
...
func _set_mastery_level(res: PlayerResource, category: StringName, level: int) -> void:
	...
	entry["level"] = level
	entry["xp"] = 0
	# Drop spent picks that no longer fit the new point budget (admin level-down).
	var tree: MasteryTreeResource = MasteryService.tree_for(category)
	if tree != null:
