...
func _authentication_callback(peer_id: int, data: PackedByteArray) -> void:
	...
	if is_valid_authentication_token(auth_token):
		...
		# Level-99 mastery titles. attach() wires the live signal for the rest of
		# the session; sync() is the retroactive sweep that catches accounts which
		# capped a skill before this system existed, and any XP path that ever
		# slips past the signal. Both are idempotent, so this costs one dictionary
		# walk per login and grants nothing on the second pass.
		SkillMasterTitleService.attach(connected_players[peer_id])
		...
		if not earned_titles.is_empty():
			...
			database.save_player(connected_players[peer_id])
