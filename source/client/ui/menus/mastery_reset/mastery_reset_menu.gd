extends Control
## Confirm dialog for a weapon-mastery re-spec — reached from Horizon's
## MasteryResetInteraction. Same compact card as attribute / skill-perk reset.
...
func _on_confirm() -> void:
	var result: Array = await Client.request_data_await(
		&"mastery.respec", {}, InstanceClient.current.name
	)
	hide()
	if result[1] != OK:
		return
	...
	match str(data.get("reason", "")):
		...
		"nothing":
			Toaster.toast("You haven't spent any mastery points yet.")
		_:
