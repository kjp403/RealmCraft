...
##
## Every named set gets its OWN recoloured sheet, generated by
## tools/paperdoll/recolor_gear.py, so an Astral Robe renders as an Astral Robe.
## The inventory icon is cropped from that same sheet, which is what keeps the
## picture in the bag and the armour on the body from ever disagreeing.


## Set slug for this piece: the first word of the name, lowercased and stripped of
## punctuation ("Scholar's Hood" -> &"scholars", "Astral Robe" -> &"astral").
func appearance_tier() -> StringName:
	var first: String = String(item_name).get_slice(" ", 0).to_lower()
	var slug: String = ""
...
## [tier, ""] for anything with a drawn layer, else [&"", &""]. The variant slot is
## unused for armour - the tier IS the sheet.
func appearance_item() -> Array:
	if slot == null or not PaperDoll.GEAR_LAYERS.has(slot.key):
		return [&"", &""]
	return [appearance_tier(), &""]


## Inventory icon cropped from the worn sheet, when one exists. Falls back to the
## authored [member item_icon] for pieces with no tier art (rings, relics, ammo).
func display_icon() -> Texture2D:
	var path: String = "res://assets/sprites/characters/gear_icons/%s_%s.png" % [
		appearance_tier(), slot.key if slot != null else &""
	]
